...
public class AlphaBetaSearch<S, A, P> implements AdversarialSearch<S, A> {
    ...
    public final static String METRICS_DEPTH = "treeDepth";
    ...
    public A makeDecision(S state) {
        ...
        List<A> actions = game.getActions(state);
    ...
    public double maxValue(S state, P player, double alpha, double beta) {
    	metrics.incrementInt(METRICS_DEPTH);

    	if (metrics.getInt(METRICS_DEPTH) >= 10) {
    		metrics.decrementInt(METRICS_DEPTH);
